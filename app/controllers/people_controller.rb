...
#  named_scope :auto, lambda { { :conditions => { :locale => I18n.locale } } }
  ...
  show_action :closest do
    ...
    @c = params[:ignore_press]
    cc = "notign"
    if @c
       cc = "ign"
    end
       
    @this = @a.path_to(@b,cc) if @a and @b
  end
...
=end
	...
	@interpersons = case @people_sort
	when 'related_person' then
		InterpersonalRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join people on interpersonal_relations.related_person_id = people.id", :order => "people.name ASC")
	when 'p_to_p_relation_type' then
		InterpersonalRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join p_to_p_relation_types p2 on interpersonal_relations.p_to_p_relation_type_id = p2.id", :order => "p2.name ASC")
	when 'organization' then
		InterpersonalRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "left outer join organizations on interpersonal_relations.organization_id = organizations.id", :order => "organizations.name ASC")
	when 'information_source' then
		InterpersonalRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join information_sources on interpersonal_relations.information_source_id = information_sources.id", :order => "information_sources.name ASC")
	end	
...
=end
	...
	when 'start_time','end_time' then
		@person_to_orgs = PersonToOrgRelation.apply_scopes(:order_by => @org_sort, :person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page])	
	when 'organization' then
		@person_to_orgs = PersonToOrgRelation.ordered(@org_sort).apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=> params[:page])
		#Ezt a sort a */@cx érti miért nem működik... :(
		#@person_to_orgs = PersonToOrgRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join organizations on person_to_org_relations.organization_id = organizations.id", :order => "organizations.name ASC")
	when 'p_to_o_relation_type' then
		@person_to_orgs = PersonToOrgRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join p_to_o_relation_types on person_to_org_relations.p_to_o_relation_type_id = p_to_o_relation_types.id", :order => "p_to_o_relation_types.name ASC")
	when 'information_source' then
		@person_to_orgs = PersonToOrgRelation.apply_scopes(:person_id_is => @this.id).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join information_sources on person_to_org_relations.information_source_id = information_sources.id", :order => "information_sources.name ASC")
	end		
  ...
  index_action :list do
    params[:page] ||= 1
    @people_sort = params[:sort] || "name"
	case @people_sort
	when "name","updated_at","interpersonal_relations_count","person_to_org_relations_count"  then 
		@people = Person.relations_counter_is_not(0).apply_scopes(:order_by => parse_sort_param(@people_sort)).paginate(:per_page=>20, :page=>params[:page])
	when "information_source" then 
		@people = Person.relations_counter_is_not(0).paginate(:per_page=>20, :page=>params[:page], :joins => "inner join information_sources on people.information_source_id = information_sources.id", :order => "information_sources.name ASC")		
	end    
...
=begin 
	  sort = case params['sort']
			   ...
			   when "interpersonal_relations_count" then "interpersonal_relations_count"
			   when "-interpersonal_relations_count" then "-interpersonal_relations_count"
			   when "person_to_org_relations_count" then "person_to_org_relations_count"
			   when "-person_to_org_relations_count" then "-person_to_org_relations_count"
			   when "information_source" then "information_source"
			   end
  
  
	#@people        = Person.order_by(:information_source, 'desc').paginate(:per_page=>10, :page=>params[:page])
    #@people = Person.relations_counter_is_not(0).apply_scopes(:order_by => parse_sort_param(:name, :updated_at, :relations_counter, :selected_organization, :information_source)).paginate(:per_page=>10, :page=>params[:page])
	@people = Person.relations_counter_is_not(0).apply_scopes(:order_by => sort).paginate(:per_page=>20, :page=>params[:page])
=end
end
